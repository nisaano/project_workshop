...
function handleImageUpload(event) {
    ...
    if (file) {
        alert(`Изображение "${file.name}" загружено. В реальном приложении здесь будет распознавание текста.`);
        // Временная заглушка
        if (elements.sourceMaterial) {
            elements.sourceMaterial.value = `[Текст, распознанный из изображения "${file.name}"]\n\nЗдесь будет автоматически распознанный текст с вашего изображения. В реальном приложении этот текст будет извлечен с помощью OCR технологии.`;
        }
    }
}
