...
function saveUserData() {
    ...
    if (currentUser) {
        ...
        if (userIndex !== -1) {
            ...
            users[userIndex].name = state.user.name; // Сохраняем имя пользователя
...
function updateUserDisplay() {
    ...
    if (userBtn && state.user.name) {
        userBtn.textContent = state.user.name;
    }
}
...
async function generateSmartNotesAI() {
    ...
    try {
        ...
        // В случае ошибки просто очищаем поле или оставляем как есть
        ...
        alert('AI обработка временно недоступна. Пожалуйста, попробуйте позже.');
    } finally {
...
function showSettingsModal() {
    // Загружаем актуальные данные пользователя из state
